...
const COLORS = [
  { bg: "rgba(102, 0, 102, 0.85)", border: "rgba(180, 0, 180, 1)" },
  { bg: "rgba(0, 120, 200, 0.85)", border: "rgba(0, 180, 255, 1)" },
  { bg: "rgba(200, 80, 0, 0.85)", border: "rgba(255, 120, 0, 1)" },
  { bg: "rgba(0, 160, 100, 0.85)", border: "rgba(0, 220, 140, 1)" },
  { bg: "rgba(180, 180, 0, 0.85)", border: "rgba(255, 255, 0, 1)" },
];
...
export default function TheAsk() {
  ...
  const allocationData = {
    labels: a.allocations.map((x) => x.label),
    datasets: [
      {
        data: a.allocations.map((x) => x.value),
      ...
      <div ref={ref} className="section-hidden">
        <div className="text-center mb-12">
          ...
          <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-black">
            {a.title}<span className="ia-highlight">IA</span>{a.titleSuffix}
          </h2>
        ...
        >
          <div className="text-xs uppercase tracking-widest text-yellow-400 mb-2">{a.roundLabel}</div>
          <div className="font-display text-4xl md:text-6xl font-black neon-text mb-2">
            R$ 500.000
          </div>
          <div className="text-purple-300 text-sm md:text-base">{a.runwayDesc}</div>
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div className="flex flex-col items-center">
            ...
            <div className="text-center mt-4">
              ...
              <div className="text-xs text-purple-400">{a.chartCenter}</div>
            </div>
          </div>

          <div className="space-y-3">
            {a.allocations.map((item, idx) => (
              <div
                key={item.label}
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: COLORS[idx].border, boxShadow: `0 0 8px ${COLORS[idx].border}` }}
                />
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start gap-2">
                    <span className="font-semibold text-sm text-white leading-tight">{item.label}</span>
                    <span className="font-display font-bold text-base flex-shrink-0" style={{ color: COLORS[idx].border }}>
                      R$ {item.value}k
                    </span>
                  </div>
                  ...
                  <div className="mt-2 h-1.5 rounded-full bg-purple-900 overflow-hidden">
                    <div
                      ...
                      style={{ width: `${item.pct}%`, backgroundColor: COLORS[idx].border }}
                    />
                  </div>
                  <div className="text-xs text-purple-500 mt-0.5">{item.pct}{a.pctOf}</div>
                </div>
              </div>
            ))}
          </div>
